            ...
            <body>
                <div class="top-bar">
                    ...
                    <input type="text" class="url-input" id="urlInput" 
                           ...
                           onkeydown="if(event.key==='Enter') loadUrlInPanel()">
                    <button class="go-btn" onclick="loadUrlInPanel()">Load in Panel</button>
                    <button class="toggle-btn" onclick="toggleBrowserPane()" id="toggleBtn">Hide Browser</button>
                </div>
...
Use the URL bar above to load websites in the browser panel.

Note: Some websites may not load due to iframe restrictions.
For full compatibility, use 'browser <url>' command to open in new window.
                    ...
                    <div class="browser-pane" id="browserPane">
                        <div class="browser-header">🌐 Browser Panel</div>
                        <div class="browser-placeholder" id="browserPlaceholder">
                            <h2>Browser Panel</h2>
                            <p>Enter a URL above and click "Load in Panel" to display a website here.</p>
                            <p><strong>Note:</strong> Some websites block iframe embedding for security.</p>
                            <p>If a site doesn't load, use the terminal command: <code>browser &lt;url&gt;</code></p>
                        </div>
                        <iframe class="browser-webview" id="browserFrame" src="" style="display: none;"></iframe>
                    </div>
                </div>
                
                <div class="status-bar">
                    <span>Terminal: <span id="terminalStatus" class="disconnected">Connecting...</span></span>
                    <span class="browser-info">Browser panel supports iframe-friendly sites</span>
                </div>
                ...
                <script>
                    ...
                    function loadUrlInPanel() {
                        ...
                        // Show loading state
                        placeholder.innerHTML = '<h2>Loading...</h2><p>Loading: ' + url + '</p>';
                        ...
                        browserFrame.onerror = function() {
                            placeholder.innerHTML = '<h2>Failed to Load</h2><p>Could not load: ' + url + '</p><p>This website may block iframe embedding.</p><p>Try using terminal command: <code>browser ' + url + '</code></p>';
                        ...
                        addToTerminal('🌐 Loading in browser panel: ' + url + '\\n');
                    }
                    ...
                    function doResize(event) {
                        if (!isResizing || !browserPaneVisible) return;
...
• browser <url> - Open URL in separate browser window
• Any other shell command will be executed

Browser Panel:
• Use the URL bar above to load iframe-compatible sites
• Some sites block iframes - use 'browser <url>' for full compatibility
...
"""
    ...
    def create_window(self):
        ...
        print("Creating split-screen window...")
        self.window = webview.create_window(
            'Terminal & Browser Split Screen',
            'http://localhost:8080',
            width=1400,
            height=900,
            min_size=(1000, 600)
